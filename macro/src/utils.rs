...
pub fn take_item_assets_attrs<Attr>(item: &mut impl MutItemAttrs) -> syn::Result<Vec<Attr>>
where
  Attr: syn::parse::Parse,
{
  let mut pallet_attrs = Vec::new();

  while let Some(attr) = take_first_item_assets_attr(item)? {
...
pub fn take_first_item_assets_attr<Attr>(item: &mut impl MutItemAttrs) -> syn::Result<Option<Attr>>
where
  Attr: syn::parse::Parse,
{
  ...
  if let Some(index) = attrs.iter().position(|attr| {
    attr
      ...
      .segments
      .first()
      .map_or(false, |segment| segment.ident == "asset")
  }) {
